...
void TerrainRenderResources::buildPipelines() {
  NSString* shaderSource = @"using namespace metal;"
    ...
    "  constexpr float kInset = 0.000001;"
    "  constexpr float kUvMax = 0.99999;"
    "  const bool isShadow = in.tileOrigin.x < 0.0;"
    "  if (isShadow) {"
    "    const float shadowAlpha = clamp(in.color.y, 0.0, 1.0);"
    "    const float2 p = in.uv * 2.0 - float2(1.0, 1.0);"
    "    const float r = length(p);"
    "    const float mask = 1.0 - smoothstep(0.72, 1.0, r);"
    "    const float a = shadowAlpha * mask;"
    ...
    "  const float2 uvLocal = uvRepeats ? clamp(uvWrapped, float2(0.0), float2(kUvMax)) : clamp(in.uv, float2(0.0), float2(kUvMax));"
